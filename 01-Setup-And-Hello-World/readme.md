...
`go mod tidy`|Verify existing module dependencies: Adds missing and removes unused modules
...
- Define the executable code in `main.go`
...
- `fmt` is from the standard library so it is already available
  ...
  - For 3rd-Party libraries, we use `go get`
...
:-|:-
`go build`|**Compiles** a bunch of Go source code files into executable binaries
`go run`|**Compiles and executes** a bunch of Go source code files (build + run) *but does not produce an actual executable (compiles in memory only)*
...
- Running `go vet` on the above code will capture the error where no value was specified for the placeholder of `Printf()`
  - The code will still compile and run
  - However, it is not correct
- **NOTE: Remember to always run `go vet` before compiling/building and pushing to version control**
  - This is one of the first step to ensure quality code
...
- *Target Definitions*
  ...
  - **After `:` is a requirement target that must be ran before running the current target**
- `.PHONY` helps avoid possible name-collisions with other directory or file names on the computer
